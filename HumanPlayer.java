 ...
 * Ce joueur peut interagir avec le plateau en choisissant des actions via des entrées clavier.
 */
public class HumanPlayer extends Player {
    ...
    /**
     * Permet au joueur humain de choisir une action parmi les options disponibles.
     *
     * @param board L'état actuel du plateau de jeu.
     */
    public void chooseAction(Board board) {
        Game.display.out.println("Voici les actions possibles : ");
        ...
        Game.display.out.println("Entrez 3 : Prendre trois jetons de types de ressources différents");
        ...
        if (Integer.valueOf(choix) < 1 || Integer.valueOf(choix) > 4) {
            Game.display.out.println("Erreur : Votre réponse n'est pas comprise entre 1 et 4.");
            Game.display.out.println("");
            chooseAction(board);
        ...
        // Gérer les jetons excédentaires
        chooseDiscardingTokens(board);
    }

    /**
     * Gère la suppression des jetons excédentaires si le joueur dépasse la limite autorisée.
     *
     * @param board L'état actuel du plateau de jeu.
     */
    public void chooseDiscardingTokens(Board board) {
        while (super.getNbTokens() > 10) {
            ...
            Game.display.out.println("Saisir 3 pour jeter: émeraudes");
            ...
            // Parcours des ressources pour jeter les jetons
            for (int i = 1; i <= 5; i++) {
                if (Integer.valueOf(choix) == i) {
                    if (super.getNbResource(Resource.values()[i - 1]) > 0) {
                        ...
                        if (Integer.valueOf(choix) < 1 || Integer.valueOf(choix) > super.getNbResource(Resource.values()[i - 1])) {
                            Game.display.out.println("Erreur : Nombre invalide pour jeter des ressources.");
                            Game.display.out.println("");
                            chooseDiscardingTokens(board);
                            return;
                        } else {
    ...
    /**
     * Permet au joueur de choisir une carte à acheter sur le plateau.
     *
     * @param board L'état actuel du plateau de jeu.
     */
    public void chooseCard(Board board) {
        ...
        Game.display.out.println("");
        Game.display.out.println("Pour sélectionner votre carte, écrivez sous le format suivant : x/y");
        Game.display.out.println("Avec x le numéro de la ligne (entre 1 et 3)");
        Game.display.out.println("Avec y le numéro de la colonne (entre 1 et 4)");
        Game.display.out.println("");
        ...
        if (Integer.valueOf(choix1) < 1 || Integer.valueOf(choix1) > 3) {
            Game.display.out.println("Erreur : Votre x doit être compris entre 1 et 3.");
            Game.display.out.println("");
            chooseCard(board);
            return;
        }
        ...
        if (Integer.valueOf(choix2) < 1 || Integer.valueOf(choix2) > 4) {
            Game.display.out.println("Erreur : Votre y doit être compris entre 1 et 4.");
            Game.display.out.println("");
            chooseCard(board);
            return;
        }

        // Validation de l'achat de la carte
    ...
    /**
     * Permet au joueur de choisir une ressource pour recevoir deux jetons de ce type.
     *
     * @param board L'état actuel du plateau de jeu.
     */
    ...
    /**
     * Permet au joueur de choisir trois jetons de ressources de types différents.
     *
     * @param board L'état actuel du plateau de jeu.
     */
    public void chooseManyRes(Board board) {
        ...
        Game.display.out.println("Saisir 2 pour revenir au choix de base à tout moment");
        Game.display.out.println("");
        for (int i = 0; i < 5; i++) {
